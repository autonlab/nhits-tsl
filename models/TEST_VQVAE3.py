...
class EncoderDecoderTransformer(nn.Module):
    """
    Standard Seq2Seq Transformer Structure.
    Encoder: Processes Past Tokens (Self-Attention)
    Decoder: Generates Future Tokens (Self-Attention + Cross-Attention to Encoder)
    """
    def __init__(self, hidden_dim, num_heads, num_encoder_layers, num_decoder_layers, dim_feedforward, dropout=0.1):
        super().__init__()
        
        # PyTorch Standard Transformer Module
        ...
        # Max length assumed 1000 for safety
        self.past_pos_embedding = nn.Parameter(torch.randn(1, 1000, hidden_dim)) 
        self.future_pos_embedding = nn.Parameter(torch.randn(1, 1000, hidden_dim))

    def forward(self, src, tgt):
        """
        src: [Batch, Hidden, N_in]  <- Past Tokens
        tgt: [Batch, Hidden, N_out] <- Future Queries (Placeholders)
        """
        # 1. Permute to [Batch, Seq, Hidden] for Transformer
        src = src.permute(0, 2, 1) # [B, N_in, H]
        tgt = tgt.permute(0, 2, 1) # [B, N_out, H]
        
        B, N_in, H = src.shape
        _, N_out, _ = tgt.shape
        
        # 2. Add Positional Embeddings
        # Broadcasting [1, 1000, H] -> [B, N, H]
        src = src + self.past_pos_embedding[:, :N_in, :]
        tgt = tgt + self.future_pos_embedding[:, :N_out, :]
        
        # 3. Transformer Pass (Encoder + Decoder)
        # Output: [B, N_out, H]
        output = self.transformer(src, tgt)
        
        # 4. Restore Shape [Batch, Hidden, N_out]
...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        self.transformer = EncoderDecoderTransformer(
            ...
            num_encoder_layers=self.configs.e_layers, # e_layers   
            num_decoder_layers=self.configs.d_layers, # d_layers
